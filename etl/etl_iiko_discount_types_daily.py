"""
ETL: типы скидок по дням из iiko OLAP в Neon (таблица discount_types_daily_iiko).
...
def fetch_discount_types(token: str, date_from: dt.date, date_to: dt.date):
    ...
    filters = {
        ...
        "OrderType": {"filterType": "IncludeValues", "values": ["Delivery by courier", "Доставка самовывоз"]},
    }
    body = {
        "reportType": "SALES",
        "groupByRowFields": ["OpenDate.Typed", "Department", "OrderDiscount.Type"],
        "aggregateFields": ["DishSumInt", "DiscountSum", "UniqOrderId.OrdersCount"],
        "filters": filters,
    }
    ...
    for r in data.get("data", []):
        ...
        disc_type = (disc_type or "").strip() or "Без скидки"
        rows.append({
            "department": dep,
            "oper_day": oper_day,
...
def upsert_discount_types(conn, rows):
    ...
    sql = """
        INSERT INTO discount_types_daily_iiko (
            department, oper_day, discount_type, orders_count, revenue, discount_sum, updated_at
        )
        VALUES %s
    ...
    """
    values = [
        (r["department"], r["oper_day"], r["discount_type"], r["orders_count"], r["revenue"], r["discount_sum"])
        for r in rows
    ]
    with conn.cursor() as cur:
        execute_values(cur, sql, values, template="(%s,%s,%s,%s,%s,%s,now())", page_size=500)
    conn.commit()
